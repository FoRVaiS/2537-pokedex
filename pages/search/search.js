...
(() => {
    ...
    const processSearchQuery = async function processSearchQuery(ref, modeOptions) {
        ...
        // Form validation
        if (query.length === 0)
            return sendAlert('An input of an integer or string is required.');
        if (!validateQueryInput(query))
            return sendAlert(`"${query}" is not a valid input.`);
    ...
    searchRef.addEventListener('keypress', e => {
        if (e.key === 'Enter')
            processSearchQuery(searchRef, modeOptions);
    });
